---
description: Lite paper — high-level, user-centric explanation of how Boros works
...
- **floating interest** = the current funding rate, **always in APR**, being determined by **an external oracle**
...
> **Note:** The parameter values used in this example (liquidation incentive, margin rates) are illustrative only and may not reflect current production values. For actual production parameters, refer to `risk/global/params-values-production.md` and the per-market TOML files in `risk/market-params/`.
