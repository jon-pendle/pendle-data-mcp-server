...
def register_tools(mcp: FastMCP):
    ...
    _CHANGELOG = (
        ...
        "\n"
        "1. 2026-04-10 — Sync pipeline: add incentive columns to boros user_market_metric and pendle pool_metrics\n"
        "2. 2026-04-08 — Sync pipeline: add price column to coinglass market_funding_rate\n"
        "3. 2026-04-07 — Sync pipeline: add market status/anomaly columns to boros market_metrics_all_in_one_daily\n"
        "4. 2026-04-06 — Sync pipeline: add cross_chain_swap_intents_curated table to pendle catalog\n"
        "5. 2026-04-03 — Sync pipeline: add incentive token amount columns to user_eod_position_summary\n"
        "6. 2026-04-03 — Keep both user_stats_per_pool_daily v1 and v2 with use-case guidance\n"
        "7. 2026-04-02 — Sync pipeline: add properties column to frontend_tracking catalogs\n"
        "8. 2026-04-02 — Add user_stats_per_pool_daily_v1 catalog with per-action-type metrics\n"
        "9. 2026-03-31 — Add twitter_engagement product (tweet_engagement_delta_hourly, tweet_basic_infos_latest_hourly)\n"
        "10. 2026-03-31 — Merge per-product table_detail tools into single get_table_detail(product_id, table_name)\n"
    )
